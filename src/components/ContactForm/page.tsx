...
>
            ...
            {({ setFieldValue, isSubmitting }) => (
              <Form className="space-y-6">
                ...
                {/* File Upload Field */}
                <div>
                  <label htmlFor="storyFile" className="block text-sm font-medium mb-2">Upload Your Story</label>
                  <input
                    type="file"
                    name="storyFile"
                    onChange={(event) => {
                      setFieldValue("storyFile", event.currentTarget.files?.[0] || null);
                    }}
                    className="w-full px-4 py-3 rounded-lg bg-gray-700 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                     // Disable if needed
                  />
                ...
                <div>
                  <button
                    type="submit"
                    className="w-full py-3 bg-indigo-600 rounded-lg text-white font-semibold hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    disabled={isSubmitting || isFormDisabled} // Disable if needed
                  >
                ...
                {statusMessage.message && (
                  <div className={`mt-4 text-center ${statusMessage.type === 'error' ? 'text-red-400' : 'text-green-400'}`}>
                    {statusMessage.message}
                  </div>
                )}
